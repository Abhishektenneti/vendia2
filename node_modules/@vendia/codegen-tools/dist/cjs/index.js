...
var operations = `
  ...
  mutation _contracts__add(
    ...
    $syncMode: Vendia_SyncMode = NODE_LEDGERED
  ) {
  ...
  query _deployments__list($filter: Vendia_DeploymentInfo_FilterInput_, $nextToken: String, $limit: Int) {
    listVendia_DeploymentInfoItems(filter: $filter, limit: $limit, nextToken: $nextToken) {
...
function generateListOperation({ entity, entities, schemaNamespace }) {
  ...
  return `
          query ${entity}${queryNameSeparator}list($nextToken: String, $limit: Int, $filter: ${filterType}, $order: ${orderType}) {
            list${entityPlural}(nextToken: $nextToken, limit: $limit, filter: $filter, order: $order, readMode: NODE_LEDGERED) {
...
function getTopLevelScalarOrObjectTemplate({ prefix, type, topLevelType, hasAcl, entity, schemaNamespace, entities }) {
  ...
  return `mutation ${entity}${queryNameSeparator}${prefix} ($input: ${inputType}!, $syncMode: Vendia_SyncMode = NODE_LEDGERED${conditionParamType}${aclParamType}) {
...
function getTopLevelArrayTemplate({ prefix, type, topLevelType, hasAcl, entity, schemaNamespace, entities }) {
  ...
  return `mutation ${entity}${queryNameSeparator}${prefix} (${idParamType} $input: ${inputType}!, $syncMode: Vendia_SyncMode = NODE_LEDGERED${conditionParamType}${aclParamType}) {
...
function generateDeleteOperation({ entity, entities, schemaNamespace }) {
  ...
  return `mutation ${entity}${queryNameSeparator}${operationName} (${idParamType} $syncMode: Vendia_SyncMode = NODE_LEDGERED, $condition: ${schemaNamespace}_${entity}_ConditionInput_) {
...
var generateClient = async ({
  ...
  await compileTs(indexFilePath, version, { outDir: dotVendiaClientPath, tsconfig: tsconfigPath, dts: true });
